...
$(function () {
  ...
  Turtle.x = [$( window ).width() / 2];
  Turtle.y = [$( window ).height() / 2];
  Turtle.lineWidth = [];
  ...
  // make variables for each turtle icon
});
...
Turtle.step = function (command, values) {
  switch (command) {
    ...
    case 'CR': //Add new turtle to the turtles div
      if (values[0] in Turtle.turtles) {
        ...
        $("#turtle"+Turtle.Id).css({
          "position": "absolute",
          "left": (Turtle.WIDTH-Turtle.Size)/2+"px",
          "top": (Turtle.HEIGHT-Turtle.Size)/2+"px"
        });
    ...
    case 'FD':  // Forward
      ...
      if (xPos > Turtle.WIDTH-24){
        xPos = Turtle.WIDTH-24;
        Turtle.x[turtId] = Turtle.WIDTH-Turtle.Size/2;
      } else if (xPos < 0){
        xPos = 0;
        Turtle.x[turtId] = Turtle.Size/2;
      } else{
      ...
      if (yPos > Turtle.HEIGHT){
        ...
        Turtle.y[turtId] =Turtle.HEIGHT-Turtle.Size/2;
      } else if (yPos < 0){
        yPos = 0;
        Turtle.y[turtId] = Turtle.Size/2;
      } else{
    ...
    case 'MT': // Move 
      ...
      if (newX > (Turtle.WIDTH-Turtle.Size)/2){
        newX = (Turtle.WIDTH-Turtle.Size)/2;
      } else if (newX < -(Turtle.WIDTH-Turtle.Size)/2){
        newX = -(Turtle.WIDTH-Turtle.Size)/2;
      }
      // Check to make sure newY is in the bounds of the canvas.
      if (newY > (Turtle.HEIGHT-Turtle.Size)/2){
        newY = (Turtle.HEIGHT-Turtle.Size)/2;
      } else if (newY < -(Turtle.HEIGHT-Turtle.Size)/2){
        newY = -(Turtle.HEIGHT-Turtle.Size)/2;
      }

      var tt = $("#turtle"+turtId);
      tt.css("left",(newX+(Turtle.WIDTH-Turtle.Size)/2)+"px");
      tt.css("top",(newY+(Turtle.HEIGHT-Turtle.Size)/2)+"px");
      Turtle.ctxTurtle.strokeStyle = Turtle.strokeStyle[turtId];
      Turtle.ctxTurtle.fillStyle = Turtle.fillStyle[turtId];
      if (! Turtle.beginFillValue){
        if (Turtle.penDownValue[turtId]) {
        ...
        Turtle.x[turtId] = newX + Turtle.WIDTH/2;
    ...
    case 'DS': // Stamp
      ...
      $("#turtle"+"_"+Turtle.stampNum+turtId).css({
        "position": "absolute",
        "left": Turtle.x[turtId]-Turtle.Size/2+"px",
        "top": Turtle.y[turtId]-Turtle.Size/2+"px"
      });
